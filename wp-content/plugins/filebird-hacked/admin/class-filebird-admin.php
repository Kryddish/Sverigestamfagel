...
class FileBird_Admin
{
    ...
    public function filebird_add_init_media_manager($hook)
    {
		...
		<div id="filebird_sidebar" style="display: none;">
			...
			<div class="filebird_sidebar panel-left"
				<?= ($sidebar_splitter_width && !$isCallModal ? ' style="width: ' . $sidebar_splitter_width . 'px;"' : '') ?>
			>
				...
				>
					...
					<div id="njt-filebird-defaultTree" class="filebird_tree">
						<ul>
							<li id="menu-item-all" data-jstree='{"selected":true}' id="menu-item-all" data-id="all" data-number="<?= ($all_count ? $all_count : '') ?>" class="menu-item">
								<svg
							...
							<li id="menu-item--1" data-jstree='{"icon":"icon-archive"}' id="menu-item--1" data-id="-1" <?= $uncatetory_count ? "data-number={$uncatetory_count}" : ''; ?> class="menu-item uncategory">
							<svg
							xmlns="http://www.w3.org/2000/svg"
							xmlns:xlink="http://www.w3.org/1999/xlink"
    ...
    private function build_folder($folders)
    {
        ...
        foreach ($folders as $k => $v) {
	        ...
	        <li id="menu-item-<?= $v->term_id; ?>" data-id="<?= $v->term_id; ?>" <?= $this->filebird_folder_counter($v->count, $v->term_id); ?> class="menu-item menu-item-depth-<?= $depth; ?>">
	        	<i class="dh-tree-icon"></i>
	            <div class="menu-item-bar jstree-anchor">
	                <div class="menu-item-handle">

	                    <span class="item-title "><span class="menu-item-title"><?= $v->name; ?></span>
	                </div>
	            </div>
	            <ul class="menu-item-transport"></ul>
	            <input class="menu-item-data-db-id" type="hidden" name="menu-item-db-id[<?= $v->term_id; ?>]" value="<?= $v->term_id; ?>">
	            <input class="menu-item-data-parent-id" type="hidden" name="menu-item-parent-id[<?= $v->term_id; ?>]" value="<?= $v->parent; ?>" />
	        </li>
	        <?php
        }
        echo '</ul></form>';
